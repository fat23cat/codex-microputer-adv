...
constexpr size_t kPayload = 61;
constexpr size_t kMaxJson = 4096;
...
class Assembler {
public:
    Assembler() { input_.reserve(512); }

    template <typename Handler>
    FeedResult feed(const uint8_t* body, size_t length, Handler&& handler)
    {
        if (!body || length < 2 || body[0] != kChannel) return FeedResult::Invalid;
        const size_t count = body[1];
    ...
    size_t buffered() const { return input_.size(); }
